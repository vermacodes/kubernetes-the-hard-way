...
In this lab you will use your computer as jumpbox. I am using Ubuntu running on my WSL. This machine will be used to run commands throughout this tutorial. To keep this document close to original, the `jumpbox` throught this repository will be used for my Ubuntu on WSL.
...
Now that you are logged into the `jumpbox` machine, you will install the command line utilities that will be used to preform various tasks throughout the tutorial.
...
```bash
git clone --depth 1 \
  https://github.com/vermacodes/kubernetes-the-hard-way.git
```
...
```text
/home/<username>/kubernetes-the-hard-way
```
...
```bash
{
  JUMPBOX_ARCH=$(dpkg --print-architecture)
  wget -q --show-progress \
    --https-only \
    --timestamping \
